...
For the purposes of `SignalBase` missing values are considered
...
Indicates whether a value is infinite. Differs from `Base.isinf` in that
`missing` is reported as finite (rather than `missing`). This method is not
exported and should generally be used without importing it to avoid confusion
with the meaning in `Base` (e.g. `SignalBase.isinf(x)`).
